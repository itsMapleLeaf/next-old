...
export function doBBCShortcut(text: string, event: Event) {
  // TODO: somehow fix this please
  const tag = jwerty.is('ctrl+shift+h', event)
    ? 'sub'
    : jwerty.is('ctrl+shift+j', event)
      ? 'sup'
      : jwerty.is('ctrl+shift+b', event)
        ? 'b'
        : jwerty.is('ctrl+shift+i', event)
          ? 'i'
          : jwerty.is('ctrl+shift+u', event)
            ? 'u'
